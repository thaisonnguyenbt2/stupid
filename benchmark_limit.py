...
for i in range(50, len(df_m5)-20):
    ...
    if signal:
        # REVERSE it
        reversed_dir = 'SHORT' if signal == 'LONG' else 'LONG'
        limit_price = row['close'] - (0.60 * row['atr']) if reversed_dir == 'LONG' else row['close'] + (0.60 * row['atr'])
...
print(f"Generated {len(signals)} REVERSED limit signals.")
...
for s in signals:
    ...
    # Check limit execution within 3 bars (15 mins)
    executed = False
    exec_price = 0
    for j in range(i+1, min(i+4, len(df_m5))):
        ...
        if signal == 'LONG' and l_row['low'] <= limit_price:
            executed = True
            exec_price = limit_price # Assumed filled at limit
            eval_start_idx = j
            break
...
print(f"\n--- Reverse Mode (30% Pullback Limit) | 1.5:1.5 R:R ---")
